...
describe("Test the filter component", () => {
    ...
    it("Should receive filterData props", () => {
        const store = mockStore({ filter: filterData });
        const wrapper = shallow(<Provider store={store}><Filter filterData={filterData} /></Provider>);
        expect(wrapper.find(Filter).prop('filterData')).toEqual(filterData);
    });

    it("Should trigger year filter change", () => {
        const store = mockStore(state);
        const wrapper = mount(<Provider store={store}><Filter filterData={filterData} /></Provider>);
        wrapper.find('#yearFilter').simulate('change', { target: { value: '2020' } });
